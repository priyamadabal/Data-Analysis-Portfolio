# Priya Madabal – Data Analytics Portfolio

Welcome to my data analysis portfolio! I'm a Cloud Engineer turned Data Analyst with a Master’s in Analytics from RMIT University.

Here you'll find a collection of my data analytics projects that showcase skills in data cleaning, visualization, forecasting, and storytelling using tools like Python, SQL, Power BI, R, and Tableau.

---

## Featured Projects

### 1. Forecasting Public Transport Demand  
**Tools**: Python, R, Power BI  
Forecasted public transport usage using ARIMA models.  
🔗 [View Project Folder](./1-Forecasting-Public-Transport-Demand)

### 2. Melbourne Real Estate Market Analysis  
**Tools**: R, Tableau, Excel  
Used regression analysis to uncover housing price trends.  
🔗 [View Project Folder](./2-Melbourne-Real-Estate-Analysis)

### 3. Retail Database Management System  
**Tools**: SQL, SQLite  
Optimized a retail database system to improve query performance.  
🔗 [View Project Folder](./3-Retail-Database-Management)

---

## Contact  
📍 Bengaluru, India  
✉️ priyamadabal1@gmail.com  
🔗 [LinkedIn](https://www.linkedin.com/in/priya-madabal-7b917b204/)
